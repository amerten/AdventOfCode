...
def score(recipe):
    calories = sum(ingredients[i][4] * recipe[i] for i in range(len(ingredients)))
    return 0 if calories != 500 else np.prod([max(0, sum(ingredients[i][j] * recipe[i] for i in range(len(ingredients)))) for j in range(4)])
